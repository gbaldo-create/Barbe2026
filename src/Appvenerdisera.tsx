import { useState, useMemo, ReactNode, FormEvent, useEffect, useRef, DragEvent, ChangeEvent, MouseEvent } from 'react';
...
const FAMILY_MEMORIES_BASE = [
  ...
  {
    text: 'Quando nacque Emma, la terza nipote, papà la guardava come se fosse la cosa più preziosa del mondo. Senza dirlo, naturalmente.',
    author: 'Aleria',
    initials: 'AL',
    context: 'La terza nipote di Franco',
    itemId: null as string | null,
  ...
  {
    text: 'Il cronometro per il tragitto Torino-Barberino era una cosa seria. Quattro ore e dieci era il record. Non ricordiamo chi lo batteva, ma ognuna giurava di avercela fatta.',
    author: 'Olivia',
    initials: 'OL',
...
async function analyzeImageWithClaude(imageBase64: string): Promise<Partial<typeof emptyForm> | null> {
  ...
  if (!base64 || base64.length < 100) { console.error('Base64 vuoto o invalido'); return null; }
...
const emptyForm = {
  ...
  price: '', catawikiUrl: '', imageUrl: '', technicalNotes: '', wearCondition: '', shipping: '',
...
export default function App() {
  ...
  useEffect(() => { sessionStorage.setItem('b2026_view', view); window.scrollTo({ top: 0, behavior: 'smooth' }); }, [view, selectedItem?.id]);
  ...
  useEffect(() => {
    const anyOpen = isItemModalOpen || isLoginModalOpen || isHeroModalOpen || isGithubTokenModalOpen;
  ...
  }, [isItemModalOpen, isLoginModalOpen, isHeroModalOpen, isGithubTokenModalOpen]);
              ...
              <section className="relative min-h-screen flex items-end overflow-hidden mb-24">
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-6 w-full pb-44 md:pb-20">
                  <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="max-w-md md:max-w-xl lg:max-w-[50%] bg-heritage-cream/60 backdrop-blur-xl p-6 md:py-5 md:px-10 lg:py-6 lg:px-12 rounded-[1.5rem] md:rounded-[2rem] shadow-2xl border-l-[4px] md:border-l-[8px] border-heritage-gold space-y-2 md:space-y-3">
                    ...
                    <div className="pt-1">
                      <button onClick={() => handleBackToCatalog()} className="px-6 py-3 bg-heritage-ink text-white rounded-lg font-bold uppercase tracking-widest text-[11px] md:text-[12px] hover:bg-heritage-gold transition-all shadow-xl group flex items-center gap-3">
              ...
              <div className="max-w-7xl mx-auto px-4 md:px-6">
                ...
                <div className="mt-12 md:mt-16 relative pb-8">
                  <div className="relative flex flex-col md:flex-row md:items-end justify-between mb-10 md:mb-16 gap-4">
                    ...
                    <button onClick={() => handleBackToCatalog()} className="group text-[11px] font-bold uppercase tracking-widest text-heritage-gold flex items-center gap-3 hover:opacity-70 transition-opacity self-start md:self-auto">
                          ...
                          <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-white rounded-2xl overflow-hidden border border-heritage-ink/8 flex flex-col">
                            {photo ? (
                              <div className="h-52 overflow-hidden">
                                <img src={photo} alt={mem.author} className={`w-full h-full object-cover ${i === 0 ? 'object-top' : i === 2 ? 'object-[center_30%]' : 'object-center'}`} />
                              </div>
                            ) : (
                            ...
                            <div className="p-6 flex flex-col flex-1 justify-between gap-5">
                              <p className="text-[16px] leading-relaxed text-heritage-ink/90 italic font-heritage">{mem.text}</p>
                              <div className="flex items-center gap-3 border-t border-heritage-ink/8 pt-5">
                                <div className="w-8 h-8 rounded-full bg-heritage-ink text-heritage-gold flex items-center justify-center text-[12px] font-bold flex-shrink-0">{mem.initials}</div>
                                <div>
                                  <p className="text-[14px] font-bold text-heritage-ink">{mem.author}</p>
                                  <p className="text-[11px] tracking-[0.15em] uppercase text-heritage-gold font-bold mt-0.5">{mem.context}</p>
                                </div>
                              </div>
                            </div>
                          </motion.div>
          ...
          {view === 'catalog' && (
            <motion.div key="catalog" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-7xl mx-auto px-4 md:px-6 pt-32 md:pt-[320px] pb-12">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="hidden md:flex flex-wrap gap-2 items-center">
                    ...
                    {categoriesWithCount.map(cat => (
                      <button key={cat.name} onClick={() => setSelectedCategory(cat.name)} className={`px-4 py-2 rounded-full text-[12px] uppercase tracking-widest font-bold transition-all flex items-center gap-2 ${selectedCategory === cat.name ? 'bg-heritage-olive text-white' : 'bg-white/50 text-heritage-ink/65 hover:bg-heritage-cream'}`}>
                  ...
                  <div className="hidden md:flex flex-wrap gap-2 items-center ml-auto">
                    <span className="text-[12px] uppercase tracking-widest font-bold text-heritage-ink/60 mr-2">Stato</span>
                  ...
                  <div className="flex md:hidden w-full items-center justify-between px-2">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[12px] uppercase tracking-widest font-bold text-heritage-ink italic truncate max-w-[200px]">{selectedCategory}{selectedStatus !== 'Tutti' ? ` • ${selectedStatus}` : ''}</span>
                    </div>
              ...
              <div className="flex flex-wrap gap-4 md:gap-8">
                ...
                {filteredItems.length === 0 && <div className="w-full py-20 text-center border border-dashed border-heritage-ink/10 rounded-2xl"><p className="text-heritage-ink/65 italic font-serif">Nessun oggetto trovato.</p></div>}
              </div>
            </motion.div>
          )}
          ...
          {view === 'item-detail' && currentItem && (
            <motion.div key={currentItem.id} initial={{ opacity: 0, y: -40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} className="pb-12 bg-heritage-cream">
              ...
              <div className="md:hidden relative" style={{marginTop: '-1px'}}>
                <div className="relative overflow-hidden bg-heritage-ink/10" style={{aspectRatio: "3/4"}}>
              ...
              {(prevItem || nextItem) && (
                <div className="md:hidden flex items-center justify-between px-4 py-3 border-b border-heritage-ink/8 bg-heritage-cream">
                  {prevItem ? (
                    <button onClick={() => { setSelectedItem(prevItem); setView('item-detail'); localStorage.setItem('b2026_selected_item', prevItem.id); }} className="flex items-center gap-1.5 text-[11px] uppercase tracking-widest font-bold text-heritage-ink/50 hover:text-heritage-ink transition-colors">
                      <ArrowLeft size={13} />
                      <span className="truncate max-w-[120px]">{prevItem.name.split(' ').slice(0,3).join(' ')}</span>
                    </button>
                  ) : <div />}
                  {nextItem ? (
                    <button onClick={() => { setSelectedItem(nextItem); setView('item-detail'); localStorage.setItem('b2026_selected_item', nextItem.id); }} className="flex items-center gap-1.5 text-[11px] uppercase tracking-widest font-bold text-heritage-ink/50 hover:text-heritage-ink transition-colors">
              ...
              <div className="hidden md:flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-2">
                  {prevItem && <button onClick={() => { setSelectedItem(prevItem); setView('item-detail'); localStorage.setItem('b2026_selected_item', prevItem.id); }} className="flex items-center gap-1.5 px-4 py-2 bg-heritage-ink/5 hover:bg-heritage-ink/10 rounded-full text-sm font-bold uppercase tracking-widest text-heritage-ink/60 transition-all"><ChevronLeft size={16} /><span>{prevItem.name.split(' ').slice(0,2).join(' ')}</span></button>}
                  {nextItem && <button onClick={() => { setSelectedItem(nextItem); setView('item-detail'); localStorage.setItem('b2026_selected_item', nextItem.id); }} className="flex items-center gap-1.5 px-4 py-2 bg-heritage-ink/5 hover:bg-heritage-ink/10 rounded-full text-sm font-bold uppercase tracking-widest text-heritage-ink/60 transition-all"><span>{nextItem.name.split(' ').slice(0,2).join(' ')}</span><ChevronRight size={16} /></button>}
                </div>
              </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="prose prose-stone mb-12">
                      ...
                      <div className="mt-4 flex flex-wrap gap-3">
                        {currentItem.wearCondition && <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-widest bg-heritage-cream border border-heritage-ink/10 text-heritage-ink/70">🔍 {currentItem.wearCondition}</span>}
                        {currentItem.shipping && <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-widest bg-heritage-cream border border-heritage-ink/10 text-heritage-ink/70">📦 {currentItem.shipping}</span>}
                      </div>
                    </div>
                    {/* Stories */}
          ...
          <div className="absolute inset-0 flex flex-col justify-end">
            ...
            <div className="relative z-10 p-4 lg:p-8 md:translate-y-4 md:group-hover:translate-y-0 transition-transform duration-500">
              ...
              <div className="relative">
                <h4 className="text-white font-serif leading-[1.2] mb-1 text-base lg:text-2xl">{item.name}</h4>
                <p className="text-heritage-gold text-[11px] lg:text-[12px] uppercase tracking-[0.2em] font-bold delay-75">{item.estimatedValue || item.price || 'Su Richiesta'}</p>
              </div>
            </div>
          </div>
        )}
    ...
    <div className="border border-heritage-ink/8 rounded-2xl overflow-hidden">
      ...
      {open && (
        <div className="p-5 space-y-4 border-t border-heritage-ink/8 bg-heritage-cream/10">
          ...
          <div className="space-y-1"><label className="text-[12px] uppercase tracking-widest font-bold text-heritage-ink/70">Stato di usura <span className="opacity-50 normal-case tracking-normal font-normal text-sm">(opzionale)</span></label><select value={form.wearCondition} onChange={e => setForm((p: any) => ({ ...p, wearCondition: e.target.value }))} className="w-full bg-transparent border-b border-heritage-ink/20 py-2.5 focus:outline-none text-lg font-heritage italic"><option value="">— non specificato —</option><option value="Ottimo">Ottimo — come nuovo</option><option value="Buono">Buono — qualche segno d'uso</option><option value="Discreto">Discreto — usura visibile</option><option value="Da restaurare">Da restaurare</option></select></div>
          <div className="space-y-1"><label className="text-[12px] uppercase tracking-widest font-bold text-heritage-ink/70">Spedizione <span className="opacity-50 normal-case tracking-normal font-normal text-sm">(opzionale)</span></label><select value={form.shipping} onChange={e => setForm((p: any) => ({ ...p, shipping: e.target.value }))} className="w-full bg-transparent border-b border-heritage-ink/20 py-2.5 focus:outline-none text-lg font-heritage italic"><option value="">— non specificato —</option><option value="Spedizione disponibile">Spedizione disponibile</option><option value="Solo ritiro diretto">Solo ritiro diretto</option><option value="Spedizione non disponibile">Spedizione non disponibile</option></select></div>
          ...
          {/* Gallery foto aggiuntive */}
          <div className="pt-2">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[12px] uppercase tracking-widest font-bold text-heritage-ink/70">Foto aggiuntive</span>
              <div className="flex gap-3">
                <label className="flex items-center gap-1 text-[11px] uppercase tracking-widest font-bold text-heritage-gold cursor-pointer">
                  <Camera size={13} /> Scatta
                  <input type="file" className="hidden" accept="image/*" capture="environment" onChange={e => { const f = e.target.files?.[0]; if (f) handleDetailImages({ target: { files: [f] } } as any); }} />
                </label>
                <label className="flex items-center gap-1 text-[11px] uppercase tracking-widest font-bold text-heritage-gold cursor-pointer">
                  <Plus size={13} /> Galleria
                  <input type="file" className="hidden" accept="image/*" onChange={e => { const f = e.target.files?.[0]; if (f) handleDetailImages({ target: { files: [f] } } as any); }} />
                </label>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              {form.images.map((img: string, i: number) => (
                <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border border-heritage-ink/10">
                  <img src={img} className="w-full h-full object-cover" />
                  <button type="button" onClick={() => setForm((p: any) => ({ ...p, images: p.images.filter((_: string, j: number) => j !== i) }))} className="absolute inset-0 bg-red-500/70 flex items-center justify-center">
                    <Trash2 size={14} className="text-white" />
                  </button>
                </div>
              ))}
              {form.images.length === 0 && <p className="text-[12px] italic text-heritage-ink/40 w-full text-center py-3 border border-dashed border-heritage-ink/10 rounded-xl">Nessuna foto aggiuntiva</p>}
            </div>
          </div>
        </div>
      )}
...
function ItemModal({ isOpen, onClose, onSave, initialData, onDelete, nextOrder }: { isOpen: boolean; onClose: () => void; onSave: (item: HeritageItem) => Promise<void>; initialData?: HeritageItem | null; onDelete?: (id: string) => void; nextOrder: number; }) {
  ...
  const handleImageUpload = async (src: string | File) => {
    ...
    try {
      ...
      // Always keep base64 for AI analysis
  ...
  const handleAnalyze = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const result = await analyzeImageWithClaude(imageBase64ForAnalysis || form.imageUrl);
      if (result) {
        setForm(p => ({
          ...p,
  ...
  return (
    <AnimatePresence>
      {/* Backdrop */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-heritage-ink/60 backdrop-blur-sm" onClick={onClose} />
        ...
        <div className="bg-white w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl pointer-events-auto" onClick={e => e.stopPropagation()}>
          ...
          <form onSubmit={handleSubmit} className="p-8 max-h-[80vh] overflow-y-auto space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2"><label className="text-[12px] uppercase font-bold tracking-widest opacity-70">Stato di usura <span className="opacity-50 normal-case tracking-normal font-normal">(opzionale)</span></label><select value={form.wearCondition} onChange={e => setForm(p => ({ ...p, wearCondition: e.target.value }))} className="w-full bg-heritage-cream/20 border-b border-heritage-ink/10 py-2 focus:outline-none text-sm"><option value="">— non specificato —</option><option value="Ottimo">Ottimo — come nuovo</option><option value="Buono">Buono — qualche segno d'uso</option><option value="Discreto">Discreto — usura visibile</option><option value="Da restaurare">Da restaurare</option></select></div>
              <div className="space-y-2"><label className="text-[12px] uppercase font-bold tracking-widest opacity-70">Spedizione <span className="opacity-50 normal-case tracking-normal font-normal">(opzionale)</span></label><select value={form.shipping} onChange={e => setForm(p => ({ ...p, shipping: e.target.value }))} className="w-full bg-heritage-cream/20 border-b border-heritage-ink/10 py-2 focus:outline-none text-sm"><option value="">— non specificato —</option><option value="Spedizione disponibile">Spedizione disponibile</option><option value="Solo ritiro diretto">Solo ritiro diretto</option><option value="Spedizione non disponibile">Spedizione non disponibile</option></select></div>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-6 space-y-6 pb-10">
          ...
          {/* ── Analizza con Claude — sempre visibile ── */}
          <div className="flex items-center gap-3 p-4 bg-heritage-gold/8 rounded-2xl border border-heritage-gold/20">
            ...
            <div className="flex-1">
              ...
              <p className="text-[13px] font-heritage italic text-heritage-ink/85 mt-0.5">Compila i campi dalla foto</p>
            </div>
            <button
              type="button"
