...
export default function App() {
  ...
  const [heroImageUrl, setHeroImageUrl] = useState(() => localStorage.getItem('b2026_hero') || 'https://images.unsplash.com/photo-1516483638261-f4dbaf036963?auto=format&fit=crop&q=80&w=2000');
  ...
  const handleAddStory = () => {
    ...
    persist(items.map(i => i.id === currentItem.id ? { ...i, stories: [...(i.stories || []), story] } : i));
    setNewStoryText('');
  ...
  const handleSaveItem = async (itemData: HeritageItem) => {
    ...
    showNotif(editingItem ? 'Oggetto aggiornato ✓' : 'Oggetto aggiunto ✓');
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-heritage-cream">
      ...
      <div className="fixed top-5 md:top-10 left-0 right-0 z-50 px-4 md:px-8 flex justify-center pointer-events-none">
        <header className="w-full max-w-6xl pointer-events-auto bg-emerald-950/90 backdrop-blur-xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[2rem] md:rounded-[3rem] text-white">
      ...
      <main className="flex-1 overflow-x-hidden">
        <AnimatePresence>
          ...
          {view === 'home' && (
            <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full">
              ...
              <div className="max-w-7xl mx-auto px-4 md:px-6">
                <section className="max-w-4xl mx-auto px-6 mb-32 space-y-12 text-center">
                  <div className="space-y-6">
                    <h3 className="text-4xl md:text-5xl font-serif italic text-heritage-ink leading-[1.1]">Una Storia di <span className="text-emerald-950 not-italic font-display font-medium tracking-tight">Famiglia</span>, <br /><span className="text-heritage-gold">un Patrimonio di <span className="text-emerald-900 not-italic font-display font-medium">Memorie</span></span></h3>
                    <div className="w-24 h-px bg-heritage-gold mx-auto opacity-40" />
                  </div>
                  <div className="space-y-8 text-lg font-light leading-relaxed text-heritage-ink/80 italic font-heritage">
                    <p>Tutto è iniziato in una splendida casa a Barberino di Mugello. Tra quelle mura abbiamo respirato storia per generazioni. Oggi, le strade della vita ci portano altrove, ma non vogliamo che questo patrimonio di bellezza e vissuti vada perduto.</p>
                    <p>Ogni mobile, ogni quadro, ogni piccolo oggetto è stato testimone di compleanni, vigilie di Natale e pomeriggi silenziosi. Ecco perché abbiamo creato questo archivio: per far sì che queste storie non finiscano, ma trovino nuovi capitoli da scrivere.</p>
                  </div>
                </section>

                <section className="max-w-7xl mx-auto px-6 mb-32">
                  <div className="grid grid-cols-2 gap-8 max-w-2xl mx-auto">
                    {[{ label: 'Oggetti Memorabili', value: mergedItems.length }, { label: 'Anni di Storia', value: '160+' }].map((s, i) => (
                      <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="text-center">
                        <div className="text-5xl md:text-7xl font-serif text-heritage-ink mb-2 italic">{s.value}</div>
                        <div className="text-[9px] uppercase tracking-widest font-bold opacity-40">{s.label}</div>
                      </motion.div>
                    ))}
                  </div>
                </section>
                ...
                <div className="mt-32 relative py-20 overflow-hidden">
                  <div className="relative flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                    <div className="space-y-4">
                      <span className="text-heritage-gold text-[10px] uppercase tracking-[0.3em] font-bold">In evidenza</span>
                      <h3 className="text-4xl md:text-5xl font-serif italic text-heritage-ink leading-tight">La <span className="text-emerald-950 not-italic font-display font-medium tracking-tight">Collezione</span></h3>
                      <div className="w-20 h-px bg-heritage-gold/30" />
                    </div>
                    <button onClick={() => handleBackToCatalog()} className="group text-[10px] font-bold uppercase tracking-widest text-heritage-gold flex items-center gap-3 hover:opacity-70 transition-opacity">
                  ...
                  <div className="relative grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 auto-rows-[240px] md:auto-rows-[320px] lg:auto-rows-[300px]">
                ...
                <section id="how-it-works" className="mt-10 md:mt-20 pt-6 md:pt-12 border-t border-heritage-ink/10">
                  <div className="max-w-4xl mx-auto text-center mb-20 space-y-4 px-6">
                    <span className="text-heritage-gold text-[10px] uppercase tracking-[0.3em] font-bold">Il nostro approccio</span>
                    <h2 className="text-4xl md:text-5xl font-serif italic text-heritage-ink leading-tight">Un Passaggio di <span className="text-emerald-950 not-italic font-display font-medium tracking-tight">Testimone</span></h2>
                    <div className="w-20 h-px bg-heritage-gold/30 mx-auto" />
                  </div>
                  <div className="grid md:grid-cols-3 gap-12">
                    {[{ icon: <Heart className="text-heritage-gold" size={32} />, t: "L'Eredità dei Ricordi", b: "Non vendiamo semplici arredi, ma affidiamo i ricordi di una vita intera. Ogni pezzo è il custode di una storia antica." }, { icon: <Handshake className="text-heritage-gold" size={32} />, t: "Vendita ed Adozione", b: "Attraverso percorsi di vendita o di adozione consapevole, ogni oggetto vi viene consegnato così come il tempo lo ha preservato." }, { icon: <MapPin className="text-heritage-gold" size={32} />, t: "Un Nuovo Inizio", b: "Tutti i pezzi della collezione sono custoditi presso i nostri locali. Vi invitiamo a scoprirli di persona." }].map((c, i) => (
                      <div key={i} className="space-y-4">{c.icon}<h3 className="text-2xl font-serif italic text-heritage-ink">{c.t}</h3><p className="text-heritage-ink/60 font-light leading-relaxed italic font-heritage">{c.b}</p></div>
                    ))}
                  </div>
                </section>
              </div>
          ...
          {/* ── CATALOG ── */}
          {view === 'catalog' && (
            <motion.div key="catalog" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-7xl mx-auto px-4 md:px-6 pt-32 md:pt-48 pb-12">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                <div>
                  <h2 className="text-3xl md:text-5xl font-serif italic text-heritage-ink leading-tight mb-2">La <span className="text-emerald-950 not-italic font-display font-medium tracking-tight">Collezione</span></h2>
                  <p className="text-heritage-ink/60 italic font-heritage text-sm md:text-base">Oggetti che hanno fatto la nostra storia, pronti per una nuova vita.</p>
                </div>
              </div>

              <div className="sticky top-28 md:top-[160px] z-40 bg-heritage-cream/90 backdrop-blur-md pt-4 pb-4 mb-10 border-b border-heritage-ink/5">
      ...
      {/* Hero modal */}
      <HeroImageModal isOpen={isHeroModalOpen} onClose={() => setIsHeroModalOpen(false)} initialUrl={heroImageUrl} onSave={url => { setHeroImageUrl(url); localStorage.setItem('b2026_hero', url); setIsHeroModalOpen(false); showNotif('Sfondo aggiornato'); }} />
...
function ItemModal({ isOpen, onClose, onSave, initialData, onDelete, nextOrder }: { isOpen: boolean; onClose: () => void; onSave: (item: HeritageItem) => Promise<void>; initialData?: HeritageItem | null; onDelete?: (id: string) => void; nextOrder: number; }) {
  ...
  const handleImageUpload = async (src: string | File) => {
    ...
    try { const r = await resizeImage(src); setForm(p => ({ ...p, imageUrl: r })); } catch { alert("Errore immagine."); }
  ...
  const handleDetailImages = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    for (const f of Array.from(files)) { if (f.type.startsWith('image/')) { try { imgs.push(await resizeImage(f)); } catch {} } }
