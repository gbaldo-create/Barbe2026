...
async function getGitHubToken(): Promise<string | null> {
  return localStorage.getItem('b2026_github_token') || import.meta.env.VITE_GITHUB_TOKEN || null;
}
...
export default function App() {
  ...
  // ── GitHub token state ──
  const [githubToken, setGithubToken] = useState<string | null>(() => localStorage.getItem('b2026_github_token') || import.meta.env.VITE_GITHUB_TOKEN || null);
  ...
  const handleLogin = (username: string, pwd: string, token?: string) => {
    if (USERS[username] && USERS[username] === pwd) {
      ...
      if (token) {
        if (token && token !== import.meta.env.VITE_GITHUB_TOKEN) {
          localStorage.setItem('b2026_github_token', token);
        }
        setGithubToken(token || import.meta.env.VITE_GITHUB_TOKEN || null);
      }
        ...
        <AnimatePresence>
          ...
          {view === 'home' && (
            <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full">
              ...
              <section id="how-it-works" className="w-full py-16 md:py-24 bg-heritage-ink mt-20 md:mt-28">
                <div className="max-w-7xl mx-auto px-4 md:px-6">
              ...
              <section className="w-full mt-20 md:mt-28 py-16 md:py-24 bg-[#EDE4D0]">
                <div className="max-w-7xl mx-auto px-4 md:px-6">
...
function LoginForm({ onLogin }: { onLogin: (username: string, pwd: string, token?: string) => void }) {
  ...
  const [token, setToken] = useState(() => localStorage.getItem('b2026_github_token') || import.meta.env.VITE_GITHUB_TOKEN || '');
...
function MobileDetailsSection({ form, setForm }: { form: any; setForm: any }) {
...
function ItemModal({ isOpen, onClose, onSave, initialData, onDelete, nextOrder }: { isOpen: boolean; onClose: () => void; onSave: (item: HeritageItem) => Promise<void>; initialData?: HeritageItem | null; onDelete?: (id: string) => void; nextOrder: number; }) {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => { if (e.changedTouches[0].clientY - touchStartY.current > 80) onClose(); };
    ...
    <AnimatePresence>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-6 space-y-6 pb-10">
          ...
          <MobileDetailsSection form={form} setForm={setForm} />
